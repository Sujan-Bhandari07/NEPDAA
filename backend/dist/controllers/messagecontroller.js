...
const sendmessage = async (req, res) => {
    try {
        ...
        let filetype = "";
        if (mes) {
            url = (await cloudinary.uploader.upload(mes.path, { resource_type: "auto" })).secure_url;
            if (url) {
...
const editmessage = async (req, res) => {
    try {
        ...
        const messages = await Message.findOneAndUpdate({ _id: messageid, sender: myid }, { message: message }, { new: true });
        if (messages) {
